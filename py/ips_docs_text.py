...
CROSSWALK_NOTES: dict[str, str] = {
    ...
    "geo":
        "GeoSPARQL carries the coordinates. This export used to emit none, "
        "on the grounds that the published discovery-site dataset already "
        "has a position for these places and a second one would compete "
        "with it. That reasoning held for a graph meant only to be merged "
        "alongside the published file, and it fails for the two things "
        "built on this one: a map, and a query page that runs rdflib in "
        "the browser. Neither can federate out to another dataset, so "
        "silence there is not neutrality but an empty map. The coordinates "
        "are therefore emitted by default on a geometry node of their own, "
        "typed geo:Geometry and sf:Point, carrying prov:wasDerivedFrom to "
    ...
    "sf":
        ...
        "defers to for the shape itself. Only sf:Point is used. It is here "
        "because tiger types the same discovery sites that way, and two "
        "graphs describing one place should not disagree about what kind "
        "of thing its geometry is.",
    "prov":
...
# --------------------------------------------------------------------------
# Colour axes and interval relations
# --------------------------------------------------------------------------
# Written after the dictionary above rather than inside it, because the
# per-axis and per-relation terms are generated from the same tables the
# exporter mints them from. Spelling them out by hand would mean that adding
# an axis to ips_colour.AXES silently produced an undocumented property —
# which make_docs.py would then refuse to build, at the far end of the
# pipeline and with no hint as to where the entry belonged.

import ips_allen as _allen          # noqa: E402
import ips_colour as _colour        # noqa: E402

TERM_DOCS.update({
    "ColourAxis":
        "A complete specification of how one measured quantity becomes a "
        "colour: the ramp stops, the interpolation between them, the "
        "domain the values are stretched onto and whether that domain is a "
        "fixed convention or was read off this corpus. Colour is "
        "presentation rather than evidence, so it is published here on the "
        "same terms as the other presentation-only quantities: as "
        "something a consumer can check and recompute rather than has to "
        "trust.",
    "hasColourAxis":
        "Relates a figure to a colour axis its rows can be shaded by. A "
        "figure carries several, because sharpness, chronology, dispersion "
        "and sample size answer different questions and choosing between "
        "them belongs to whoever is drawing the map.",
    "colourSource":
        "The measured property an axis takes its values from. Following it "
        "leads from the colour back to the evidence.",
    "normalisedProperty":
        "The property under which this axis's normalised value appears on "
        "a plot row. Named in the graph so that a client can discover the "
        "axes rather than having the property names compiled into it.",
    "hexProperty":
        "The property under which this axis's colour appears on a plot "
        "row. See normalisedProperty for why it is named rather than "
        "assumed.",
    "rampName":
        "The name of the colour scale. Deliberately distinct from "
        "colourRamp, which holds the name of a D3 function that this "
        "export does not reproduce. A ramp named here is fully specified "
        "by its stops and its interpolation, and the resulting colours are "
        "reproducible from the graph without any library.",
    "rampStops":
        "The anchor colours of the scale, as hex values separated by "
        "spaces, running from the low end of the domain to the high end.",
    "rampInterpolation":
        "How colours between two anchors are obtained. 'linear-rgb' means "
        "each channel is interpolated linearly. This is not what D3's "
        "spline interpolators do; the two agree at the anchors and differ "
        "by a few units between them.",
    "domainMin":
        "The value mapped to the low end of the ramp. Values below it are "
        "clamped rather than dropped.",
    "domainMax":
        "The value mapped to the high end of the ramp.",
    "domainBasis":
        "Whether the domain is a fixed convention or was observed in this "
        "corpus. The distinction matters for anyone comparing two "
        "snapshots: an observed domain moves when findspots are added, so "
        "an unchanged findspot can change colour without its dating having "
        "changed at all.",
    "domainScale":
        "Whether values are stretched linearly or logarithmically. "
        "Logarithmic where the quantity spans orders of magnitude, so that "
        "the ramp is not exhausted by a handful of large sites.",
})

for _axis, _col, _src, _ramp, _scale, _fixed, _why in _colour.AXES:
    TERM_DOCS[_colour.norm_property(_axis)] = (
        f"Presentation only. The value of lado:{_src} stretched onto the "
        f"unit interval by the axis samian:axis_{_axis}. {_why}")
    TERM_DOCS[_colour.hex_property(_axis)] = (
        f"Presentation only. The colour corresponding to "
        f"lado:{_colour.norm_property(_axis)} on the axis "
        f"samian:axis_{_axis}. Recomputable from that axis's stops and "
        f"interpolation; carried in the graph so that two maps of the same "
        f"data cannot disagree about the scale.")

for _name, _inv, _time_local, _definition in _allen.RELATIONS:
    TERM_DOCS[_allen.lado_local(_name)] = (
        f"Holds on the modelled interval: this time-span {_definition}. "
        f"Every pair of datings stands in exactly one relation of this "
        f"family, so the set is complete and can be counted. It does not "
